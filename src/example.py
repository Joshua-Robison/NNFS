...
# extract data
data = housing.data  # features -> X
target = housing.target  # targets  -> y
...
lr = NeuralNetwork(
    layers=[Dense(neurons=1, activation=Linear())], loss=MeanSquaredError(), seed=seed
)
...
trainer.fit(
    X_train_scaled, y_train, X_test_scaled, y_test, epochs=50, eval_every=10, seed=seed
)
